...
### 3.7 检测距离：站人 vs 跌倒（实测标定，非几何推算）

**核心事实：能"检测到人"的距离 ≠ 能"检测到跌倒"的距离，且 fall 距离显著更短。**
站人测的是胸肩高度（~120–150cm），跌倒测的是地面高度（~0–30cm）。贴地/躺姿目标的**回波远弱于站立目标**（RCS 小 + 地面杂波/多径），有效信号半径被腰斩——所以同一台雷达，跌倒检测半径远小于人员检测半径。

> **不要用 FOV 锥几何去推 fall 半径。** 雷达（加特兰 Calterah 4T4R）的俯仰波束方向图既不是干净的对称锥、vfov 也不是 layout 里标的 120°；真正卡住 fall 的是地面弱回波，纯几何模型会得出"fall ≥ person"的反结论。**d_person / d_fall 必须按设备+支架实测标定。**

**参数**：

| 符号 | 含义 | 来源 |
|---|---|---|
| `h` | 安装高度（cm）| layout `installHeight` |
| `a` | boresight 自水平面下倾角（度），`a = 90 − 与墙夹角` | firmware `Angle` query |
| `c` | 信号半径 = 最大斜距（cm），中心最强处可检测回波的直线距离 | 设备规格（强目标、近 boresight 的上限）|
| `d_person` | 站人有效检测**水平**半径（cm）| **实测标定** |
| `d_fall` | 跌倒有效检测**水平**半径（cm）| **实测标定**，恒 `< d_person` |

**仍然成立的几何（engine 必用）——斜距 ↔ 平面转换**：雷达物理量是斜距 `R`，判定/画图用平面（俯视）水平距 `d`，对高度 `z` 的目标：

```
R = √(d² + (h−z)²)        d = √(R² − (h−z)²)
```

engine 拿 firmware 上报点做距离闸时，链路必须是「斜距/位置 → 用 (h, z) 投影成平面 d → 比 d_fall」，不可拿 raw 斜距直接比平面阈值。

**简化标准值**（A 支架实测取整，c=800cm 信号半径；角度约定 `a = 90 − 与墙夹角`）：

```
d_person ≈ 700 cm    站人有效检测水平半径
d_fall   ≈ 500 cm    跌倒有效检测水平半径（恒 < d_person）
```

旁证：跌倒等效斜距 √(500²+200²)≈538cm ≪ c=800 —— 远没到信号半径就丢回波，证明 fall 受**贴地弱回波**限，不是斜距限、不是几何。各支架/型号可各自标定微调，工程默认即用此对。

**对 lost_track 闸的应用**（见 §16 / §17.4）：lost_track 凭"track 消失"反推跌倒，只有丢轨点落在 **d_fall** 内、雷达本能看清地面跌倒时才可信。

```
丢轨点平面距 d > d_fall(≈500cm)  →  该处即便真摔也给不出地面证据（firmware 发不出 pose=5）
                                  →  lost_track 退化为结构性盲猜 → 抑制或降级（不发 CRITICAL）
```

**不要用 presence 半径（d_person≈700，更远）当判据**——那会把"人走到 5–7m 站立仍可见、但已超出可确认跌倒距离"的正常脱跟当成跌倒。

> **实例 D523**（2026-06-03 Denver-101 误报）：人在距雷达 ~5.8m 边缘静止后丢轨（np=0），D523 不发 ExitRoom → 等待超时硬推 Fall CRITICAL；6min 后 np=1 站姿复现，证伪。5.8m > d_fall(500)，雷达本就确认不了地面跌倒 → 典型 lost_track 盲猜。详见 memory `lost_track_fall_detection_envelope_gate`。
