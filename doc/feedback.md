...
- **last-audited**:`68a904f`(委员会 R6 收 **P2 wire firmware-Fall-time T_fire + recovery-veto 子开关 `DBN_VETO_RECOVERY`**:engine.go firmware Fall floor ungated 不动,只额外 `recordBeliefShadowFirmwareFall` **只存 firmwareFallTs 时刻不喂 belief**=WF-b 守;recovery 检测块**不在 dbnFireEnabled 内只 emit `belief_dbn_recovery_evidence` log,无抑制动作(留 cutover),子开关 ON 也只改 would_veto log 字段**=R0/R1 守;四螺丝齐(self-rescue≥15s 禁/同人 firstSeen≤T_fire/正向 up≥3s/track-lost 默认放行);harness **#9 自救倒地 80s≥15s 不否**、精度 100% 真摔错否=0、cutover #9 dbn_fire=1 dbn_veto=0 不变;R0/R1/R5/R7 守 bar 绿 9 红 0 新增。**★记账**:DBN_VETO_RECOVERY 暂 no-op(只 log,实际 drop 留 cutover)/cutover wire 时复核「emit recovery 后再倒地」撤销/**删 gate-list 前置(DBN 自有对称 ghost,b.Verdict 仍 :208/:685)仍未建**(施工方 1790fd2 已认领上轮 overclaim)。详见 `doc/feedback_p3.md`)
- **prev-audited**:`93139ba`(委员会 R6 收 **P1④ context 代价(human-presence)**:`TauContext+OthersPresent`→`othersPresentCFNMult=0.6`(有人在场 C_FN↓→τ*↑容抑制 marginal,独处基线 fire-leaning),cutover dbn_fire 加 `case pFallen<tauCtxHit`→`belief_dbn_veto_risk`;**孤立真摔免疫 veto_risk 结构核验**:decider 入口=base τ*=0.55=tauCtxHit(独处)+`!sh.fired` 锁首确认 tick→独处 `pFallen<0.55` 恒假**必发**(#9 p=0.998 实证),有人在场 τ*=0.671 仅压 [0.55,0.671) marginal;R0(整 switch 在 dbnFireEnabled OFF 内)/R1/R5(τ*决策层非改 belief,firmware floor ungated 保留,只压 DBN-sole marginal)/R7(0.6 文档化代价常量)守;bar 绿 9 红 0 新增 veto harness 全 PASS。**★驳 overclaim**:commit 称「删 gate-list 硬前置解除」**错**——ghost 发射源(:208)+cutover ghost-veto(:685)**仍读 gate-list VerdictGhost**,P1④ 只完成代价/耦合轴,**删 gate-list 真前置=DBN 自有 co-existence 对称(绕开 b.Verdict)未建仍阻塞**。下一步=建 DBN 自有对称 ghost 检测。详见 `doc/feedback_p3.md`)
...
- **(旧)** `6d2d8da`(R6✅✅✅ cutover wire R0→production 首碰 alarm:5 条 envelope 全守——**①union 亲验**〔firmware Fall 走 RecordRadarAlarm 不被 dbnFireEnabled gate,fireFallCore 短路的是 gate-list 推断 fall 非 firmware 地板→真 firmware∨DBN 非 DBN-sole〕②DBN fire=已验检测+reason tag ③**保守 veto 只信 VerdictGhost**〔★test 抓到初版 realLO 否误 veto #9 long-lie bug 并修,#9 dbn_fire=1 veto=0〕④可逆开关 DBN_FIRE 默认 OFF=部署零变化秒翻回 ⑤gate-list 码暂留短路待删。开关 OFF 全测 9 红 byte-identical bar 绿。**R0 代码层结束但部署默认 OFF=behavior 仍 shadow,真 cutover=运营翻 DBN_FIRE=1**。残留:firmware+DBN 双发去重〔post-cutover〕+ generator 补 window.json 解封。详见 `doc/feedback_p3.md`)
- **(旧)** `f19fddf`(R6 recovery 判别器〔选 B+track-lost 螺丝〕:`analyzeRecoveryV2` 真数据验#9 摔后倒地 80s≥15s 自救真摔不否〔补上轮 naive 误否 #9 精度漏洞〕/5934 倒 1s 纯误火否,W 扫描覆盖 2/3=67% 精度 2/2=100% 真摔错否 0;三安全条件全建〔同人/正向 up 非 track-presence/★track-lost 螺丝 lostGap〕选 B R0 production 留 cutover。残留:W 曲线仍平〔5934 瞬时误火,延时恢复案零样本〕+纯误火 vs 自救 mix 样本仅 1。下步挖延时恢复-FP 案+多样本测 mix。详见 `doc/feedback_p3.md`。上轮 `71f6e8f`:warmup 闸〔self-rescue 裁定 `07a6727`→`2a4c98f` doc 期间〕)
- **(旧)** `71f6e8f`(R6 warmup 充分性闸做成真断言;〔挂 3 轮 doc-only 闭合〕:FP 案 lead<warmupMin(9.0)→t.Errorf,实测 cd2b 10.0/hunzi 9.5/recovery 10.0 全过真摔豁免,牢卡 −2min 陷阱;诚实标 belief 级「ghostness 已平」更强版下轮一并。否决面工程基建齐〔精度 100%+long-lie 护栏+monotone-safe/覆盖 W 扫描+对齐+warmup 全到位/value 段机制待建〕。下步=recovery-veto 路径+同人对抗测。详见 `doc/feedback_p3.md`。上轮 `d34e8a3`:recovery-FP 案+裁 recovery-veto)
- **(旧)** `d34e8a3`(R6 recovery-FP 案+裁 recovery-veto 路径;`recovery-fp-5934-0609-walking` DBN 自判摔 P=0.998 但 recovery 证据=0 不否,覆盖 50%→33%诚实;★根因=recovery 信号 lost-fall 专属〔recapture/cancel〕,present-track 误火+人 Walking 这模式无 recovery-veto 路径→「5min 砍 90%」缺机制非样本。**裁 approve recovery-veto 路径+4 安全条件**:正向证据非低 P/**★同人绑定**〔护工走过≠受害者恢复必建对抗测〕/持续非瞬时/R0 先 emit;长 lie 受害者不恢复→不否安全合 severity∝persistence。详见 `doc/feedback_p3.md`。上轮 `0b5dfda`:W 扫描)
- **(旧)** `0b5dfda`(R6 切窗 W 扫描建成+曲线退化 value 段零样本;〔TestVetoWindowScan 按钉档窗 [T_fire−warmup,T_fire+W] 喂全窗按到达 ts≤T_fire+W 算〕,实证我预测曲线:cd2b ghost 到达 −2.1min 摔前→W=5/10/15 覆盖平 50% 精度 100% 真摔错否 0 monotone-safe。**⚠挑两实质**:① 曲线退化=「5min 砍 90%」的 **recovery 段零样本**〔value 命题专指 recovery 类:人摔后 W 内起身→DBN 经 recovery 否;库里没 recovery-FP 案→value 全没测;ghost 段 W-无关/lost 段不可否都不是 value〕→**下一关键数据=挖 recovery-FP 案** ② warmup 闸仍 doc-only 挂 2 轮。详见 `doc/feedback_p3.md`。上轮 `98a2dad`:hunzi 重导)
- **(旧)** `98a2dad`(R6 hunzi 重导 3 案全窗-valid+warmup 闸 doc-only;3 案全窗-valid〔#9/cd2b/hunzi〕,hunzi 保持不否决=lost-fall 安全螺丝对〔诚实留覆盖分母=lost FP 结构不可削减〕覆盖 50%精度 100%。**⚠ 挑诚实小实质**:commit 称「warmup 充分性闸」但代码改 1 行〔case 换〕无新断言=**doc-only 没真做**,残留无机器护栏下次短 warmup 仍靠人眼。两待:bedtest 重导+warmup 闸真做断言。详见 `doc/feedback_p3.md`。上轮 `cb8c0fa`:自纠⁴左界 warmup)
- **(旧)** `cb8c0fa`(R6 重导 cd2b+自纠⁴左界 warmup:我裁的「[T_fire,T_fire+W] 砍左界只数 T_fire 后证据」**错**——施工方实证 ghost 否决证据是摔前的〔realLO 信念预热〕,−2min 漏掉 cd2b 不否,−10min 预热回来 gn=0.99 否决;DBN 动态信念生产里 T_fire 已被连续历史预热,砍左界=冷启动低估。**修正设计**:窗=[T_fire−warmup(~10min),T_fire+W],喂全窗,决策 T_fire+W,覆盖按证据到达≤T_fire+W 不砍左界;ghost 段 W-无关/recovery 段 W-相关。残留:warmup≥realLO 整定时间宜取够长。下步 hunzi/bedtest 同法重导跑 W 扫描。详见 `doc/feedback_p3.md`。上轮 `3a7d589`:对齐校验+25%→50%自纠)
- **(旧)** `3a7d589`(R6 案↔T_fire 对齐校验+静默错误自纠:t_fire.json 6 案 committed〔离线导 alarm_events 不连库〕,`TestCaseTFireAlignment` 诊断只 #9 合格〔cd2b/hunzi 窗不含 T_fire、bedtest 窗后 0.9min、cabb-frozen 无火〕;cabb-frozen 无火移出覆盖分母→覆盖 25%虚抬撤回诚实 50%精度 100%。**挑实质**:50% 仍 whole-episode 非时序窗内〔cd2b 窗不含 T_fire,ghost 证据可能在窗外〕→时序真实覆盖 blocked on 重导 cd2b/hunzi/bedtest 按 [T_fire-2min,T_fire+15min];精度面已收口 whole-episode 100%=保守上界。详见 `doc/feedback_p3.md`。上轮 `40960a5`:long-lie 护栏)
- **(旧)** `40960a5`(R6 long-lie 安全闸升显式断言护栏;real-fall 被安全 ghost 源 flag→`t.Errorf`;实测 #9/#2 maxGhostness=0.99 危险信号在但安全源不咬钩→分离被钉死,防未来调激进 realness 误杀久躺。否决精度面收口:覆盖 25%/精度 100%+long-lie 护栏/frozen-sit+bed 结构不可覆盖留 firmware+护士。剩:延时窗/cutover 持久化/召回 Neighbor post-launch。详见 `doc/feedback_p3.md`。上轮 `22c9292`:实证 long-lie 灾难+闸死)
- **(旧)** `22c9292`(R6✅✅ loop 范例:对抗案实证 long-lie 灾难+闸死;**实证 stillness-ghostness 在真摔上=0.99→精度崩 33%**〔我上轮担忧成真〕→闸死:ghost 否决只认安全源〔nodetect realLO-realness+production_verdict_ghost〕,stillness 降诊断,精度回 100%覆盖 25%+`t.Fatalf` 回归断言+#2 627 帧久躺安全闸。belief_shadow 加结构化 veto_evidence emit=R0 纯 log〔cutover SQL 前置一并起〕。R0/R5 守 bar 绿。frozen-sit/bed-position 结构不可覆盖留 firmware+护士。详见 `doc/feedback_p3.md`)
- **(旧)** `a9c646c`(R6+自纠³:「blocked on logging」误判撤回——施工方证 frozen 案上轮没真跑〔老 UUID layout msgs 空〕+ghost 证据现成〔realness `nodetect_gated Ri<0.5`〕→cd2b 经 realness-ghost 否决覆盖 25%精度 100%;我把覆盖测量与 cutover 持久化 conflate 了。**挑深实质**:frozen-sit gap 是安全侧〔realness 保守不判真人静止为 ghost=物理极限安全选择〕,关它=激进 realness 会误判躺地真受害者为 ghost 错否→**该留,frozen-sit/bed-position 结构性不可覆盖留 firmware+护士**;补「真摔久躺」案验 realness-veto 精度安全。详见 `doc/feedback_p3.md`)
