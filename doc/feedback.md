...
- **last-audited**:`93139ba`(委员会 R6 收 **P1④ context 代价(human-presence)**:`TauContext+OthersPresent`→`othersPresentCFNMult=0.6`(有人在场 C_FN↓→τ*↑容抑制 marginal,独处基线 fire-leaning),cutover dbn_fire 加 `case pFallen<tauCtxHit`→`belief_dbn_veto_risk`;**孤立真摔免疫 veto_risk 结构核验**:decider 入口=base τ*=0.55=tauCtxHit(独处)+`!sh.fired` 锁首确认 tick→独处 `pFallen<0.55` 恒假**必发**(#9 p=0.998 实证),有人在场 τ*=0.671 仅压 [0.55,0.671) marginal;R0(整 switch 在 dbnFireEnabled OFF 内)/R1/R5(τ*决策层非改 belief,firmware floor ungated 保留,只压 DBN-sole marginal)/R7(0.6 文档化代价常量)守;bar 绿 9 红 0 新增 veto harness 全 PASS。**★驳 overclaim**:commit 称「删 gate-list 硬前置解除」**错**——ghost 发射源(:208)+cutover ghost-veto(:685)**仍读 gate-list VerdictGhost**,P1④ 只完成代价/耦合轴,**删 gate-list 真前置=DBN 自有 co-existence 对称(绕开 b.Verdict)未建仍阻塞**。下一步=建 DBN 自有对称 ghost 检测。详见 `doc/feedback_p3.md`)
- **prev-audited**:`15a4151`(委员会 R6 收 **P1②发射换源+2真人单测+P1③ fall×P(Real)**〔`5599edc`+`25db34a`+`15a4151`〕:② `TObsPresent.Ghostness` 主源 单 track frozen→**co-existence 对称** `b.Verdict==VerdictGhost?0.9:0.15`,realLO 降级仅诊断,2真人案实测 ρ=0.9 无对称→P(Ghost)=0.0055 都 Real〔堵①over-flag〕;③ fall 证据 `Conf×=pReal`(`belief_shadow:261`),**R5 结构核验**:降 Conf 把 LR 推向中性永不反转<1,ghost 假 still-fall 中性化、真摔不压低,harness 真摔 #9 peakP=0.998 孤立 pReal≈1 照常 ramp、精度 100% 真摔错否=0,**R5 守**;bar 绿 9 红 0 新增 belief+veto harness 全 PASS;R0/R1/R5/R7 守。**★台账**:②③ 的 Ghostness/pReal 共同 bootstrap 自 gate-list `b.Verdict`→**删 gate-list 唯一阻塞根依赖**,删前须建 DBN 自有对称〔复用 `checkMirrorSymmetry` 几何当 feature 绕开 `b.Verdict`〕。P1 进度①②③✓剩④ C_FN 吃 human-presence。详见 `doc/feedback_p3.md`)
