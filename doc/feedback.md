...
- **last-audited**:`6d2d8da`(委员会 R6✅✅✅ 收 **cutover wire**=R0→production 转折首碰 alarm 路径:5 条 envelope 全守——**①union 亲验**〔firmware Fall 走 RecordRadarAlarm 不被 dbnFireEnabled gate,fireFallCore 短路的是 gate-list 推断 fall 非 firmware 地板→真 firmware∨DBN 非 DBN-sole〕②DBN fire=已验检测+reason tag ③**保守 veto 只信 VerdictGhost**〔★test 抓到初版 realLO 否误 veto #9 long-lie bug 并修,#9 dbn_fire=1 veto=0〕④可逆开关 DBN_FIRE 默认 OFF=部署零变化秒翻回 ⑤gate-list 码暂留短路待删。开关 OFF 全测 9 红 byte-identical bar 绿。**R0 代码层结束但部署默认 OFF=behavior 仍 shadow,真 cutover=运营翻 DBN_FIRE=1**。残留:firmware+DBN 双发去重〔post-cutover〕+ generator 补 window.json 解封。详见 `doc/feedback_p3.md`)
