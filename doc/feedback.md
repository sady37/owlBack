...
- **last-audited**:`22c9292`(委员会 R6✅✅ loop 范例:施工方建委员会点名的「真摔久躺」对抗案,**实证 stillness-ghostness 在真摔上=0.99→精度崩 33%**〔我上轮担忧成真〕→闸死:ghost 否决只认安全源〔nodetect realLO-realness+production_verdict_ghost〕,stillness 降诊断,精度回 100%覆盖 25%+`t.Fatalf` 回归断言+#2 627 帧久躺安全闸。belief_shadow 加结构化 veto_evidence emit=R0 纯 log〔cutover SQL 前置一并起〕。R0/R5 守 bar 绿。frozen-sit/bed-position 结构不可覆盖留 firmware+护士。详见 `doc/feedback_p3.md`)
