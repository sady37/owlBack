...
- **last-audited**:`8bf89e8`(委员会 R6 收 **P2 集成测 recovery-veto production 路径端到端验通(清待办①)**:test-only 生产码零改,`TestRecoveryVetoIntegration` 走 production 路径(摔前走动→firmware Fall 事件 handleEventMessage→recordBeliefShadowFirmwareFall 设 firmwareFallTs→摔后倒地→起身),**纯误火 3s<15s→recovery=1 / 自救 20s≥15s→recovery=0**(self-rescue guard 端到端生效);激活此前 dormant recovery 路径,验通 option A firmware-Fall-time 时刻参考端到端正确;仍 shadow R0(recovery_evidence 只 log,DBN_VETO_RECOVERY 默认 OFF 实际 retraction 留 cutover);bar 绿 9 红 0 新增。剩:实际 retraction wire(现 would-veto 仅 log no-op)+ cutover「emit 后再倒地」撤销复核,皆 cutover-time。详见 `doc/feedback_p3.md`)
