...
- **last-audited**:`6219700`(委员会 R6 收 **★P1-final 增量2 里程碑:ghost 检测面 DBN-native 化达成**:新 `tm.GetInterferes()`(锁+copy 线程安全)+`dbnMirrorSymmetryGhost`(self 镜像位有 VerdictReal partner≤50cm,catch cd2b 冻结);发射 :263 改 `motion||mirror` **删 `||b.Verdict`**、cutover veto :740 改 `coExist&&(motion||mirror)` **删 `fb.Verdict==VerdictGhost`**。**硬核验**:bar 绿 9 红 0 新增/-race 绿/死锁安全(beliefShadowTick 操作 bases 快照不持 tm.mu)/**harness 精度 100% 真摔错否=0**(#9 孤立必发、cd2b 经 no-detect 否非 :740)。R0(veto 在 dbnFireEnabled OFF 内)/R1/R5/R7 守。**★精化 overclaim**:称「不读 b.Verdict」过宽——self-ghost 脱 gate-list ghost 输出✓,但 partner-real 仍读 `p.Verdict==VerdictReal`(=track_manager 核心:1387/1403 设,**非 gate-list**,删后仍在)→里程碑实质成立措辞过宽。**残留 b.Verdict 3 处确为非检测**(诊断 emit+Room层 :302 生产 track 过滤,删 gate-list 后 inert)。**答 :305=后者**(gate-list 删 PR 一并退役 :302/:282,现删不安全因 gate-list 仍设 VerdictGhost)。剩:gate-list 实删=独立协调 PR(逐条验覆盖)。详见 `doc/feedback_p3.md`)
- **prev-audited**:`db9adea`(委员会 R6 收 **清 P1-final 增量1 整改单①(同向两真人单测+risk-accepted 文档化)**:test-only 生产码零改 R0/R1/R5 守 bar 绿 9 红 0 新增;`TestDBNMotionSymmetry_SideBySideTwoReal` 实跑同向两真人并排 60cm→判 ghost(上轮挑的 false-positive 实测确认);**显式文档化明确预期**:motion 对称分不开反射 vs 第二真人(继承 tm.checkMotionSymmetry),此模式判 ghost=**一切看风险 risk-accepted**(共存可救+P1④τ*↑),增量2 mirror 才真分开。**闭合上轮挑实质 #1**。小记:characterization 测(t.Logf 不硬断言)无回归闸,增量2 后宜升硬断言。剩 P1-final 增量2(mirror=删 gate-list 真前置)。详见 `doc/feedback_p3.md`)
