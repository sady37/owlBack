...
- **last-audited**:`db9adea`(委员会 R6 收 **清 P1-final 增量1 整改单①(同向两真人单测+risk-accepted 文档化)**:test-only 生产码零改 R0/R1/R5 守 bar 绿 9 红 0 新增;`TestDBNMotionSymmetry_SideBySideTwoReal` 实跑同向两真人并排 60cm→判 ghost(上轮挑的 false-positive 实测确认);**显式文档化明确预期**:motion 对称分不开反射 vs 第二真人(继承 tm.checkMotionSymmetry),此模式判 ghost=**一切看风险 risk-accepted**(共存可救+P1④τ*↑),增量2 mirror 才真分开。**闭合上轮挑实质 #1**。小记:characterization 测(t.Logf 不硬断言)无回归闸,增量2 后宜升硬断言。剩 P1-final 增量2(mirror=删 gate-list 真前置)。详见 `doc/feedback_p3.md`)
- **prev-audited**:`7e983b7`(委员会 R6 收 **P4 生成器基建 + 裁「合成测不准分类」**:test-only 生产码零改 R0/R1/R5 守 bar 绿 9 红 0 新增;诊断 moving/lost fire 0/10、silent fire 6/10 分类对 1/6(多判 pose_lying)、walk_only 检出 10/10。**裁:推 live 不精化合成造假签名**——合成是 program-完备非 correctness,**分类准确率 correctness 问题合成结构上测不了**(手搓签名=循环),silent 误判 pose_lying 非 DBN bug 是合成用躺姿块 Pose 主导 DBN 正确判 pose_lying;合本会合成-vs-真原则+用户先拆定「分类非安全 live 验」。**修正**:分类准确率非合成 gate不阻塞 cutover;pre-cutover 信号走真 donor 小 N reason 断言(harness 已有 #9 fire 加 p7_3_reason 断言),authoritative=post-cutover 护士标注。详见 `doc/feedback_p3.md`)
