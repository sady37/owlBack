...
- **last-audited**:`3a7d589`(委员会 R6 收案↔T_fire 对齐校验+静默错误自纠:t_fire.json 6 案 committed〔离线导 alarm_events 不连库〕,`TestCaseTFireAlignment` 诊断只 #9 合格〔cd2b/hunzi 窗不含 T_fire、bedtest 窗后 0.9min、cabb-frozen 无火〕;cabb-frozen 无火移出覆盖分母→覆盖 25%虚抬撤回诚实 50%精度 100%。**挑实质**:50% 仍 whole-episode 非时序窗内〔cd2b 窗不含 T_fire,ghost 证据可能在窗外〕→时序真实覆盖 blocked on 重导 cd2b/hunzi/bedtest 按 [T_fire-2min,T_fire+15min];精度面已收口 whole-episode 100%=保守上界。详见 `doc/feedback_p3.md`。上轮 `40960a5`:long-lie 护栏)
