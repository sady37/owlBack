...
- **last-audited**:`643e8d9`(委员会 R6 收 **同向两真人测升硬断言(回归闸)**:test-only 生产码零改,`TestDBNMotionSymmetry_SideBySideTwoReal` 由 `t.Logf`→`t.Errorf` 锁「同向两真人→判 ghost」=risk-accepted 回归基线,未来阈/几何翻转→红强制复核;闭合 db9adea 小记「增量2 后升硬断言」;bar 绿 9 红 0 新增 R0 守。详见 `doc/feedback_p3.md`)
- **prev-audited**:`6219700`(委员会 R6 收 **★P1-final 增量2 里程碑:ghost 检测面 DBN-native 化达成**:新 `tm.GetInterferes()`(锁+copy 线程安全)+`dbnMirrorSymmetryGhost`(self 镜像位有 VerdictReal partner≤50cm,catch cd2b 冻结);发射 :263 改 `motion||mirror` **删 `||b.Verdict`**、cutover veto :740 改 `coExist&&(motion||mirror)` **删 `fb.Verdict==VerdictGhost`**。**硬核验**:bar 绿 9 红 0 新增/-race 绿/死锁安全(beliefShadowTick 操作 bases 快照不持 tm.mu)/**harness 精度 100% 真摔错否=0**(#9 孤立必发、cd2b 经 no-detect 否非 :740)。R0(veto 在 dbnFireEnabled OFF 内)/R1/R5/R7 守。**★精化 overclaim**:称「不读 b.Verdict」过宽——self-ghost 脱 gate-list ghost 输出✓,但 partner-real 仍读 `p.Verdict==VerdictReal`(=track_manager 核心:1387/1403 设,**非 gate-list**,删后仍在)→里程碑实质成立措辞过宽。**残留 b.Verdict 3 处确为非检测**(诊断 emit+Room层 :302 生产 track 过滤,删 gate-list 后 inert)。**答 :305=后者**(gate-list 删 PR 一并退役 :302/:282,现删不安全因 gate-list 仍设 VerdictGhost)。剩:gate-list 实删=独立协调 PR(逐条验覆盖)。详见 `doc/feedback_p3.md`)
