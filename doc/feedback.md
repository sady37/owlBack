...
- **last-audited**:`498cd63`(委员会 R6 收 **redis-replay unit 级全量回放**:`--unit` 自动查 /64 prefix 所有设备+alarm 流仅 device-gateway+`--streams` alarm opt-in 保守+向后兼容;build/vet rc=0;★收 bed scorer gap 发现:sleepad event 未进 zoneengine 非 replay 工具 bug。详见 `doc/feedback_p3.md`)
