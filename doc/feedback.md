...
- **last-audited**:`713daed`(委员会 R6 收 **P1-final 增量1 DBN 自有 motion 对称 ghost(lock-free 绕开 b.Verdict 第一步)**:新 `dbnMotionSymmetryGhost(self,bases,prevPos)`——self 在动+与共存 `VerdictReal+在动` track 紧贴<100cm+同向 cos>0.866→motion 反射 ghost,**lock-free**(循环前 prevPos 快照避 ordering 坑,bases 不碰 tm 锁);发射 :260 改 `dbnMotionSymmetryGhost||b.Verdict→0.9`。**影响面只喂发射→P(Ghost)→P1③ pReal,cutover veto(:737)仍 fb.Verdict 不受影响**;R0/R1/R5/R7 守 bar 绿 9 红 0 新增 harness 全绿。**★挑实质(整改单,非阻塞)**:① **同向两真人 false-ghost**:test 正例「紧贴同向→ghost」与「两真人并排同向走」motion 不可区分→真人走动被判 ghost→pReal↓→若摔 P1③ 折扣;边界=阈值继承 tm.checkMotionSymmetry(非新)/只喂发射不喂 veto/moving-state 摔时清/共存可救(一切看风险)/shadow R0;须补同向两真人单测+明确预期 ② 仍 **OR-additive**(`||b.Verdict` 只增 ghost flag=R5 方向),施工方诚实标增量1,**b.Verdict 仍消费(:260/:737),删 gate-list 真前置待增量2 mirror 对称**(interference 几何)建完去 `||b.Verdict`。详见 `doc/feedback_p3.md`)
- **prev-audited**:`344d31b`(委员会 R6 收 **P3 moving_fall tag 功能 + 开 #1.1/#1.3 整改单**:`tlayer.lastMoveMs` 记摔前在动,cutover fire `case ok` 里 pose_lying 主导+摔前 5s 内 MoveActive→`reasonTag="dbn_moving"` 否则 `dbn_pose_lying`,写 AIPayload.Reason+log;DBN 输出全 4-tag。**tag 是 fire 后 label 不 gate 发射**=R0(case ok 在 dbnFireEnabled OFF 内)/R1/R5(不改 belief)/WF-b 守,bar 绿 9 红 0 新增。**★整改单(cutover 前必清,非安全阻塞)**:① `"dbn_moving"`(:743)是 **inline 裸字面量**,其它 3 tag 走 `"dbn_"+p7Reason.String()`(enum 源)→ **4-tag 词表分裂违 #1.1/#1.3**,须落 `ReasonMoving` 常量进 `fall_reason.go` 单源 ② **无单测**(moving/pose_lying 判别未覆盖)→ cutover 分类准确率前置须补。详见 `doc/feedback_p3.md`)
