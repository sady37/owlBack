...
- **last-audited**:`98a2dad`(委员会 R6 收 hunzi 按钉档 [T_fire−10min,+15min] 重导:3 案全窗-valid〔#9/cd2b/hunzi〕,hunzi 保持不否决=lost-fall 安全螺丝对〔诚实留覆盖分母=lost FP 结构不可削减〕覆盖 50%精度 100%。**⚠ 挑诚实小实质**:commit 称「warmup 充分性闸」但代码改 1 行〔case 换〕无新断言=**doc-only 没真做**,残留无机器护栏下次短 warmup 仍靠人眼。两待:bedtest 重导+warmup 闸真做断言。详见 `doc/feedback_p3.md`。上轮 `cb8c0fa`:自纠⁴左界 warmup)
