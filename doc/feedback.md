...
- **last-audited**:`7dd48de`(委员会 R6 收 **geom 全量退役实施**:D1-D4 签字后施工,删 belief.Geom 类型/字段/geomFrom*、GeomConf→AreaConf,代码残留 grep=NONE;逐 tick 等价零新回归(仅 3 预存红 dwell+sleepace 无关);新增 TestBedsideFallBedReleased 锁床边 FN 红线;已部署 test1(09:24,DBN_MODE=2)。详见 `doc/geom_retirement_mapping.md`§7 + `doc/feedback_p4.md`)
