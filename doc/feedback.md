...
- **last-audited**:`40960a5`(委员会 R6 收 long-lie 安全闸升一等显式断言:real-fall 被安全 ghost 源 flag→`t.Errorf`;实测 #9/#2 maxGhostness=0.99 危险信号在但安全源不咬钩→分离被钉死,防未来调激进 realness 误杀久躺。否决精度面收口:覆盖 25%/精度 100%+long-lie 护栏/frozen-sit+bed 结构不可覆盖留 firmware+护士。剩:延时窗/cutover 持久化/召回 Neighbor post-launch。详见 `doc/feedback_p3.md`。上轮 `22c9292`:实证 long-lie 灾难+闸死)
