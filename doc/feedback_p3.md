...
t=120s → d=0.5 (弱相关)
```
60s 是**矩阵收敛的单位时间步长**(一个 unit tick 内的事件彼此在相关距离内),不是 dwell 时长。tail 表(20min/12min/90min)是 zone safety baseline,与 60s 在**不同维度上**→不可比较→我上轮"20min→60s 翻哲学"质问无意义。

**纠正后的事实**:
- D523 提案的 AreaSit=90min **比当前 8min 更安全**(18min→LR=1.04 vs 当前 6.06 cap),不会恶化 ✓
- 60s 与 dwell 尾正交:一个决定事件间是否相关,一个决定相关事件在不同 zone 的 dwell 敏感度 ✓

**★新挑战(基于正确理解):60s 相关距离的正当性**

① **为什么是 60s?** d=60/t 公式的分子=60 来自哪里?是雷达 1Hz→60 帧=矩阵充分收敛?是老人动作的最小时间尺(从站立到倒地 >60s)?是医学观察(老人跌倒后尝试起身的平均时间)?**须给出来源**。

② **d=1 为什么是"相关"阈?** d≥1 表示"相关",d<1 表示"不相关"。这个阈值的物理解释是什么?两个事件相隔 61s(d<1)就应该被矩阵认为无关吗?**1 的分界是否太硬**(类似 gate-list 的硬分支)?

③ **学术界/工业界怎么用时间相关距离?** (委员会独立调研如下)

**DBN 领域基准**:Nicholson(Monash Univ,2002)的经典 DBN 跌倒检测模型——时间离散化为不规则区间(按传感器事件触发),两条相邻事件之间的时长存为 time node `T`,CPD(Conditional Probability Distribution)**是 Δt 的函数**:Δt 越大→转移概率衰减→旧证据权重降低。这是「时间相关」在 DBN 里的标准建模:不硬切窗,让概率分布本身含时间衰减项。

**工业界常用三种时间衰减函数**:

| 模型 | 公式 | 特点 | 适用场景 |
|---|---|---|---|
| **指数衰减** | w = k^t (k∈(0,1)) | 渐进趋零,永不为 0 | 置信度/证据老化(Belief Update) |
| **有限窗口** | w=1(t<W) / w=0(t≥W) | 硬截断 | 事件关联(Event Correlation) |
| **反比例衰减(d=K/t)** | d = 60/t | 近期陡降,远期平缓 | ★项目组A 选用 |

**三种模型的比较(对 fall detection)**:

| | 指数衰减(k^t) | 有限窗口 | 反比例衰减(d=60/t) |
|---|---|---|---|
| t=30s | 需选 k | 1 | 2.0(强相关) |
| t=60s | 需选 k | 1/0 | 1.0(相关) |
| t=120s | 需选 k | 0 | 0.5(弱相关) |
| t=300s | 需选 k | 0 | 0.2(极弱) |
| 参数选择 | k 需要数据标定 | W 需要领域知识 | K=60 需要领域知识 |
| 是否硬边界 | 否(渐近) | 是(阶跃) | 否(渐近) |
| 对 fall 的适用性 | **最优**(老人动作时间尺连续,不是阶跃) | 差(阶跃=另一种 gate) | 介于两者(近端陡降≈有限窗口,远端缓降≈指数,无硬 cut) |

**★关键洞见**:项目组A 选择的 **d=60/t 反比例衰减 = 有限窗口+指数衰减之间的折中**——近 t<60s 时 d>1(类似有限窗口"相关"),远 t>60s 时 d→0 但不硬 cut(类似指数衰减的渐近)。所以**比有限窗口软(无 gate),比指数衰减简单(单参数 K=60)**。这个选择在数学上是合理的。

**但 d=60/t 有一个指数衰减没有的问题**:参数 K=60 决定了「多远算无关」的**拐点位置**。在 K=60 时,拐点在 t=K(60s)=d=1。而指数衰减的 k 需要标定,拐点不在 t=1/k。**d=60/t 的 K 等价于指数衰减的 1/λ —— 都需要领域知识或数据标定**。

**对项目组A 的质问**(基于以上调研):

→ **K=60 的来源**:Nicholson 用传感器事件间隔(步行中 foot switch 的步态周期 ~0.5-1s)作 time unit。项目组A 的 60s 对应的是雷达帧(1Hz)的 60 帧——雷达帧不是"事件触发"而是"固定采样"。**为什么用 60 帧(=60s)而非更短的雷达事件间隔?** 如果 K=60 标定自「老人行为的最小时间尺」(如从站到坐到躺的时间),请给出该尺度的来源(文献或数据)。

→ **d=60/t 的 K 是否需按 roomType 分层?** Nicholson 的 CPD 是 per-node 的(不同节点的转移概率不同)。当前 proposal 的 K=60 是全局常数——但 bathroom(高频事件,toilet→stand→shower 几分钟内完成)和 bedroom(低频,睡眠持续数小时)的"事件相关时间尺"显然不同。**是否需要 roomType×K(如 bathroom K=30, bedroom K=120)?还是全局 60 足够?**
...
### [2026-06-11] ✅ 委员会博士答辩式审核——5 肯定 + 4 质疑 + 3 裁定(20min 尺度→可建但须澄清化)

**审核方法**:逐命题挑战,不凭声明,有论据。收的肯定有理据,否的也有。

---

#### 肯定部分(有理据,收)

**①「全量空间不应有硬排除」— 收,结构性对**。`default→false` 把 3/5 的 Geom(Unknown/InEnter/InBed)排除在 dwell 计算外,在一个 9×5×5×12=M·b 矩阵架构里是**结构不一致**的——其他证据(pose/vital/lost/gate)都不因 Geom 而沉默,唯独 DwellStill 因 Geom 而沉默。项目组A 称「gate-list 同质」准确:gate-list 在外部做分支→丢弃,DBN 应让矩阵内部算。**论据成立,收**。

**②「GeomInBed 仍排除」— 收,正确保留**。bed 区是躺着正常的地方,dwell 不应报 fall。**不争议,收**。

**③ LR 定量— 收,自行核算验证通过**。`1+(71/1200)²=1.0035≈1.004`,`1+(188/1200)²=1.0245≈1.02`,`1+(360/1200)²=1.09`。shape=2(Weibull k=2)使短 dwell 极缓——71s 仅抬 0.4%,**确实温和**。**论据可复现,收**。

**④「矩阵自然算出权重不靠硬分支」— 收,且 DBN 已有防御支撑此论断**。`ObsDwellStill` 的 `Conf=0.7`,经 P1③ `Conf×=pReal`——若 track 在 area=255 是 ghost,pReal 低→Conf 进一步折扣→SFallen boost 微乎其微。**ghost 防御已前置,P1③/P1② 保护在,不因加 GeomUnknown 而裸奔**。**论据成立,收**。

**⑤ 诚实预测 0606「P(Fallen)仍不达 0.55」— 收,诚实**。20min 尾下 dwell 抬极微(188s→LR=1.02,与 ObsPose(4) StandWalk 对冲,净效果近乎零)。修的是原则(不硬排除),不是声称修后 0606 必 fire。**诚实不自夸,收**。

---

#### 质疑部分(有理据,要回应)

**★质疑 1:20min 尺度的正当性——为什么是 20min,不是 8min 或 15min?**

提案称 20min=「未知区域久站与已知开旷地久站的保守平衡」。但 20min 意味着:一个人在未知区要站 20 分钟,SFallen 的 dwell 贡献才等于已知开旷地站 8 分钟的贡献(1+(480/480)²=2.0 vs 1+(480/1200)²=1.16)。**20min 的正当性来自哪里?** 是来自数据(真 case 的 dwell 分布)?还是来自原则(「未知用最保守」)?如果是后者,那 toilet 15min 已经被认为"保守"了,20min 是「比保守更保守」——对于真摔(0606 床边,人倒地后 dwell~7min),20min 尾下 LR≈1.03,几乎不可检测。**需要明确:20min 的正当性论证**(不是简单说"保守平衡")。

**★质疑 2:GeomInEnter 从「不加」翻到「默认 20min」的转变,未解释逻辑跳跃**

联合讨论阶段项目组A 明确写「建议仅加 GeomUnknown,**不加 GeomInEnter**(门区久静=等人/准备出门,不应报)」。本次提案把 GeomInEnter **和 GeomUnknown 一起放入 default 20min**。变化的原因是什么?如果是认识到「GeomInEnter 也应有弱 ramp」——那「门区久静=等人」这个原反对理由现在如何化解?20min 尾能否区分「门区久站等人」和「门区久站=摔倒后走到门区倒地」?**需要澄清这个翻转,否则是内部不一致**。

**★质疑 3:「未知区域久站 ≥ 已知开旷地久站的安全性」— 等价未证**

提案称「未知区域久站 ≥ 已知开旷地久站的安全性」作为保守设置的正当性。但**逻辑反了**:已知开旷地 = 确认无家具/床/椅子,人久站=可疑;未知区域 = 可能有家具/椅子,人久站 = 可能在坐着休息。**未知区域的安全性不比开旷地高——它是 *不确定*,不是 *更危险***。加尾的正当性应来自「不确定区域不能预设安全」(= DBN 不应替未知做判断),而非「不确定区域比已知开旷地更危险」(= 未证等价)。**须修正论证逻辑**。

**★质疑 4:固定空间论证("算得动")论证了可行性,未论证必要性**

「9×5×5×12=有限空间,算得动,不应排除」——这证明了**能**算(computational feasibility),但未证明**该**算(model correctness)。命题「因为算得动所以不该排除」混淆了计算能力和模型正确性。**正确的论证应当是**:`GeomUnknown`(area=255)在物理上对应「person is present in an unclassified region of radar FOV」——这个物理状态**没有理由被 a priori 排除在 fall dwell 模型外**。论证应改向"物理语义"而非"矩阵大小"。**须修正论证逻辑**。

---

#### 裁定

**1. 方向收(去硬 gate,给 GeomUnknown 慢尾),尺度 20min 可建但须回应质疑 1(正当性来源)。** 若来自"最保守"原则→委员会接受(合 DBN「不确定时保守 ramp」哲学)。若来自数据→需贴出真 case dwell 分布。

**2. GeomInEnter 是否入 default——须明确,不可未解释翻转。** 委员会建议:**GeomInEnter 先不入**(保守:门区久站=等人/准备出门,不应报 fall),**只加 GeomUnknown,20min 尾**。GeomInEnter 可后续数据驱动补(门区真摔 vs 门区等人的 LR 差异有数据后再议)。

**3. 论证逻辑修正(不阻塞建,但论文不该这样写)**——质疑 1/3/4 的回应纳入最终裁定文档:正当性来自"不确定≠安全"非来自"算得动",20min 来自"最保守非 0"(DBN 对未知区不预设安全)。**修后再审一次,但倾向签字**。
