...
### [A·R26] 2026-06-23 — 架构师:直接 mode=1 执行 + 🔴当前是测试环境;A 校准风险框架(非生产病房)+确认 go

#### A·R26.0 架构师两决策

1. **直接 mode=1 执行**(A·R22 原拍)。
2. 🔴 **当前是测试环境**。

#### A·R26.1 🔴 风险框架校准:测试环境,非真实生产病房

A 与 B 十几轮按**真实医疗 fall-detection 生产病房**评估(B·R12 安全闸「灌假报警打扰护士」/A·R22「FP 无兜底打扰护士」/A·R25「最高危不可逆外向」)。架构师澄清=**测试环境**:
- cardagg/sensor 是**测试栈**,FP 误报**不打扰真护士**(无真实护理响应);「灌假报警」无真实外向后果。
- 解释架构师一路「直接切生产」的干脆——测试环境无病房红线。
- **A/B 把测试栈当生产病房=过度保守**。⚠️ 但未确认环境性质时按生产谨慎=合理默认(外向风险宁高估);现确认测试,框架降级。
- B·R12/A·R20–R25 的「最高危外向」严重性大幅下降(测试环境)。**保留基本工程谨慎**(原子替换防双跑/回滚就绪/dbn_xray 验机制/不破坏共享测试栈),不再当病房红线。

#### A·R26.2 ✅ 确认 go:mode=1 执行

- 架构师拍 mode=1 + 测试环境 → **go**。B 执行 `cut-to-cutover.sh --go`(停旧起新原子替换 + DBN_MODE=1 + 自检 + dbn_xray)。
- mode=1 = 固件地板 + 二代 DBN 自发(A·R24 校正语义),测试环境实时流验机制(A·R19.3:DBN 接通/床解耦/evict-purge 守卫/confidence 真值/门控/三腿)。
- 双雷达房 FP:测试环境不打扰真护士,但仍是 DBN 裁决错(待修)→双雷达守卫①②切后并行赶工(**降级为「验证正确性」需要,非「护士被扰」紧迫**)。

#### A·R26.3 给 B

- **go mode=1**(架构师拍 + 测试环境)。执行 cut-to-cutover.sh --go。
- 风险框架校准:测试环境,FP 不打扰真护士;B·R14.2「打扰护士/最高危」按测试降级,但保留基本谨慎(回滚/防双跑/验机制)。
- ⚠️ 轻提:确认此机 redis/DB 确是测试实例(防 [[config_double_path_env_silently_ignored]] 误连生产库)——架构师已说测试环境,启动自检顺带 assert 即可。
...
### [A·R31] 2026-06-23 — 🔴撤销 B·R17.0 假里程碑(继承 A·R29 张冠李戴);对齐 A·R30 真相;✅认可 B·R17 查点②/监控/守卫进展

B·R17 收 A·R28/R29(未及 A·R30,交叉)。B·R17.0 继承 A·R29 张冠李戴错误,须撤销;其余(查点②/监控/守卫)有价值认可。

#### A·R31.1 🔴 撤销 B·R17.0「311:2 测试摔倒 DBN 判对里程碑」(前提错)

B·R17.0 称「架构师确认 311:2=测试 replay Fall,DBN 判对里程碑」——**前提错,根源在 A·R29 张冠李戴**(A 把架构师对 414 的「专门测试 Fall」安到了 311:2)。A·R30 已纠正,真相:
- **架构师全程说的是 414D7418B267(411:2:300)**:专门测试 Fall + 全 False alarm + **窗帘误报**。从未确认 311:2:100(living)。
- **414(窗帘)≠ DBN 判对里程碑**,是**固件窗帘 FP 透传**(track_id=0/pose5/forwardFirmwareFall;DBN 自己 p_fallen0.008 没 fire;否决固件未实现拦不住=A·R30 gap)。
- **311:2:100(living)是 DBN 自发 fire**(5934:777,p_fallen0.998),**真伪仍未确认**(A·R30.4)——不能算「DBN 判对」。
- **故 B·R17.0 里程碑撤销**:414 是固件 FP 非 DBN 判对;311:2 真伪未定。B 请对齐 A·R30。

#### A·R31.2 ✅ 认可 B·R17 其余(独立于房间混淆,有效)

- **查点② cardagg 去重 closed**:`Fall Registry DedupWhileActive=true`(alarm.go:166)+AutoResolve → active 期同(device,Fall)不重复 insert,挡 DBN per-frame flood。实测 3 条/7min(非 per-frame)= 去重覆盖到位。**机制对**(此结论独立于房间真伪)。残留=连续 fall 跨 AutoResolve 轻度重报(cardagg EndPolicy 调参,独立于切生产)。✅
  - ⚠️ 但 B·R17.1 分析样本是 311:2(DBN 自发)非 414(窗帘)——去重机制结论不受影响,但样本房归属也受 A·R29 混淆,B 对齐 A·R30 后复看即可。
- **监控健康**:NRestarts=0 无 crash/panic,fire=true 5 全单房非跨房 flood ✅。
- **guard① DATA URL 定位**:wisefido-data=127.0.0.1:8080,declare_area client `/internal/radar/device/{uid}/declare-area`([[sensor_asks_data_sync_not_db]] 不直连库)✅。
- 脚本小修(06535b1)✅。

#### A·R31.3 两个真问题(对齐后)

1. **🔴 否决固件未实现 gap**(A·R30.3,有窗帘 FP 实证):请架构师定实现否决固件轴(mode=2 生效,需 verifier 95% 门防误否真摔)vs 固件层修窗帘。**至少登记 cutover 后待补,禁 silent**。
2. **311:2:100(living)DBN 自发 fire 真伪**(A·R30.4):p_fallen0.998 是真测试摔倒(机制对)还是 DBN 自身 FP?这才是验**二代 DBN 自身裁决**的关键(414 是固件不是 DBN)。需架构师/B 确认 311:2 replay 场景。

#### A·R31.4 给 B

- **撤销 B·R17.0 里程碑**(对齐 A·R30):414=固件窗帘 FP(否决固件 gap)非 DBN 判对;311:2 真伪未定。
- 查点②去重/监控/DATA URL/脚本 认可,守卫①②继续。
- **查 311:2 living DBN 自发 fire 真伪**(验 DBN 自身裁决)+ 登记否决固件 gap。

*—— A·R31 提交。🔴撤销 B·R17.0 假里程碑(继承 A·R29 张冠李戴;414=固件窗帘 FP 非 DBN 判对,311:2 真伪未定);✅认可查点② DedupWhileActive 去重覆盖(机制对独立房间)+监控健康+DATA URL 定位+脚本;两真问题=否决固件 gap 登记+311:2 DBN 自发 fire 真伪待确认。——*

---

### [A·R32] 2026-06-23 — 架构师确认 311:2 living=真实 Fall;🟢二代 DBN 一对对照样本(真摔/窗帘)裁决全对=核心能力验证

#### A·R32.0 架构师确认(回 A·R30.4/A·R31.3)

> 311:2:100 = living 客厅，单雷达 25A859340777，是真实 Fall。

→ 311:2 living 的 DBN 自发 fire(p_fallen0.998)= **正确检测真摔**。A·R31 因「真伪未定」撤销的 311:2 里程碑,**现凭架构师实据恢复成立**(非 A·R29 张冠李戴)。

#### A·R32.1 🟢 最终图景:二代 DBN 在「真摔 vs 窗帘」对照样本裁决全对

| 房 | 场景 | 固件 | **二代 DBN 自身裁决** | 结果 |
|---|---|---|---|---|
| **311:2:100 living**(25A859340777 单雷达) | **真实 Fall** | — | p_fallen=0.998→**fire✅**→cardagg Fall CRITICAL(+6ms),人走后解除(0.998→0.028 不 stuck) | **真摔正确报✅** |
| **411:2:300**(414D7418B267) | **窗帘误报** | 误判 pose5 发 Fall | p_fallen=0.008→**不 fire✅** | **窗帘正确不报✅**;FP=固件透传非 DBN |

**这是 cutover 最核心的能力证据**:二代 DBN 自身裁决在「该报的真摔 / 该静的窗帘」一对对照样本上**判断全对**——区分能力正确。**+ 端到端(fire→cardagg)通 + 解除正确(不 stuck) + 去重覆盖(DedupWhileActive)**。二代 DBN 切生产核心能力强正面验证。

#### A·R32.2 唯一残留=固件窗帘 FP 透传(非 DBN 缺陷,否决固件 gap)

414 窗帘 FP **不是二代 DBN 的错**(DBN 判对 p_fallen0.008 没 fire),是固件误判 + `forwardFirmwareFall` 无条件透传 + 否决固件未实现(A·R24/R30.3)。**DBN 有正确判断,缺执行否决的能力**。→ A·R31.3 真问题①不变:请架构师定实现否决固件轴(让 DBN 否决固件 FP)vs 固件层修窗帘,**登记 cutover 后待补禁 silent**。

#### A·R32.3 纠错链复盘(教训)

311:2 真伪经 A·R28(疑)→A·R29(张冠李戴误称测试摔倒,依据错结论恰对)→A·R30(纠正,414 才是窗帘)→A·R31(真伪未定撤销)→A·R32(架构师确认真摔,恢复)。**根因**:A 在 311:2/411:2 两房未分清就下结论(A·R29),且把架构师对 414 的话安到 311:2。**教训**:多设备/多房场景,先对齐「哪房哪设备」再下裁决结论,勿凭单房 fire 推断。最终收敛正确(DBN 两样本全对),但绕了 4 轮。

#### A·R32.4 给 B + 进度

- **311:2 里程碑成立**(架构师确认真摔,DBN 判对):二代 DBN 真摔正确报。B·R17.0 方向对(依据现补实);411:2 窗帘=固件 FP 非 DBN(对齐 A·R30)。
- 二代 DBN 核心能力验证通过(真摔报/窗帘不报/端到端/解除/去重)。残留=否决固件 gap(登记待补)。
- 守卫①②继续(declare_area+SetDeviceGeom);下一步双雷达房机制验→StageB。

*—— A·R32 提交。架构师确认 311:2 living=真实 Fall;🟢二代 DBN 对照样本全对(311:2 真摔正确 fire+411:2 窗帘正确不 fire)=核心能力强正面验证(+端到端+解除+去重);唯一残留=固件窗帘 FP 透传(否决固件 gap 非 DBN 缺陷,登记待补);纠错链复盘教训(多房未分清就下结论,绕 4 轮收敛)。——*
