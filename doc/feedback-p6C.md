...
# §70 增补 — 架构师补桶二时序约束（出生窗内算一次锁定，非每帧）— 修正 §69 柱A（算力优化 + 抗噪声抖动 + FN-safe 闸口径）

> 架构师补：**mirror 占位/出生地 从出生的前 3-5 秒内算一次锁定，之后不算**（非 §69 柱A 隐含的"每帧算"）。C 核：物理依据成立，三重收益（不只省算力），但补一条 FN-safe 闸口径（防出生瞬间噪声锁错）。本节修正 §69 柱A 时序 + 补柱F，不改 §69 原文（追加纪律）。

## 一、约束 + 物理依据（架构师定，C 核成立）

**法**：IsReflection 几何只在 ghost **出生时 / 出生后前 3-5 秒窗内算一次，锁定；之后这条 track 不再重算**。

**物理依据**（C 核，与早定 firmware 时标体系一致）：firmware 能过滤的 ghost，前 3-5 秒内过滤掉（过滤完即消失）；**3-5 秒还没被 firmware 过滤掉 = 硬件过滤不了它 = 后面也永远过滤不掉**——过了这个窗，ghost 的"是不是镜像"已定性、不会变，再算是浪费算力。firmware 在前几秒做它的过滤，过滤不掉的才稳定留到本层。

## 二、C 核：三重收益（不只省算力）

1. **算力优化**：mirror 几何是**静态判定**（ghost 出生位 + 墙 + 雷达坐标，三者在 ghost 生命周期内不变）→ 算一次够。从 `O(每帧×墙数)` 降到 `O(每 track 一次×墙数)`，每帧重算是冗余。
2. **抗噪声抖动（正确性收益，非仅省算力）**：每帧重算会因雷达 XY 抖动导致"这帧判镜像、下帧判不是"的帧间抖动。出生窗算定、锁定，消除这种抖动。
3. **与既有 FN-safe 思路一致**：同噪声防线"持久化确认 K 帧"（§65）、"ghost detect ≥2 周期"（§66）——别拿单帧/瞬时定性，等稳定。

## 三、🔴 FN-safe 闸口径（C 钉死 — 防出生瞬间噪声锁错真人）

**关键：不是"出生第一帧算"，是"前 3-5 秒窗内取稳定帧锁定"**。若拿出生瞬间单帧（可能是 cd2b 式噪声尖峰）算几何 → 基于噪声坐标锁错：

| 锁错方向 | 后果 | 安全侧 |
|---|---|---|
| 锁成 `true`（真人被误锁镜像） | 排出 N_r → **漏报 FN** | ❌ 否决 |
| 锁成 `false`（镜像漏判） | 多报 → 多查 | ✅ FP 安全侧，可接受 |

**两条 FN-safe 偏置**：
- **取稳定帧锁定**（非出生首帧）：给几帧让 XY 稳定下来再锁，避免拿出生瞬间噪声帧定性（同 §64/§66"别拿瞬时定性"）。
- **窗内判不准时偏 false**：宁可窗内偏 `false`（漏镜像多报）也不偏 `true`（误真人漏报）。

## 四、修正 §69 柱A + 补柱F

- **§69 柱A 时序修正**：几何"每帧算" → **"出生窗（~3-5s）内取稳定帧算一次，锁定；之后不重算"**。几何三步（点在矩形外 + 连线求最近交点 + ≥30cm 闸）**不变**，只改时序（从 per-frame 改 per-track-once）。
- **补柱F 时序正确**：① 出生窗内算一次、锁定，之后不重算（算力 + 静态属性）；② 窗内**取稳定帧**锁定（非出生首帧），且方向 FN-safe 偏置（不准偏 false）；③ 锁定后整条 track 生命周期 IsReflection 不变。**核**：合成 case → ghost 出生首帧塞噪声尖峰坐标（像真人出生位）+ 窗内后续稳定帧落镜像位 → 应锁 true（取稳定帧、非首帧噪声）；反向，出生稳定帧落真人位、之后噪声尖峰飘镜像位 → 锁定后噪声不翻 IsReflection（已锁定）。

## 五、C 给 A（§69 五柱 + §70 柱F）

桶二 = census **出生窗一次算 IsReflection、稳定帧锁定**（§69 柱A-E 几何/FN-safe/闭环/多墙/零回归 + §70 柱F 时序）。

**C 复审重点（钉死）**：① 窗内**取稳定帧**锁定（非出生首帧），防出生瞬间噪声把真人锁成镜像 → 漏报；② 窗内不准偏 `false`（FN-safe）；③ 锁定后不重算、IsReflection 不抖。补完 ghost 三分（桶一伪迹 / 桶二镜像 / 桶三病根删）全闭环，Xsensorv1 使命才真达成。
