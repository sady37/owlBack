...
func (c *MQTTConsumer) handleMessage(topic string, payload []byte) error {
	if isQinglanVerboseLog() {
		if c.logger != nil {
			c.logger.Debug("MQTT_RX", zap.String("topic", topic), zap.String("payload", string(payload)))
		} else {
...
func (c *MQTTConsumer) handleMonitorMessage(uid string, message map[string]interface{}) error {
	...
	// 根据canMonitor标志决定如何处理监控数据
	if canMonitor {
		// CanIot=T & CanMonitor=T：正常发送监控数据，监控数据同时也作为心跳
		...
		// CanIot=T & CanMonitor=F：不发送监控数据，仅发送心跳信号
...
func (c *MQTTConsumer) handleStatMessage(uid string, message map[string]interface{}) error {
	...
	for _, m := range items {
		...
		// 根据canMonitor标志决定如何处理数据
		if canMonitor {
			// CanIot=T & CanMonitor=T：正常发送统计数据，不发送额外心跳，正常数据同时也作为心跳
			switch cat {
			...
			// CanIot=T & CanMonitor=F：不发送统计数据，仅发送心跳信号
			// 不处理统计数据，直接跳过
		}
	}
	
	// 无论canMonitor是true还是false，都需要确保发送心跳以维持设备在线状态
	// 当有人活动时，业务数据本身就作为心跳；当无人活动只有固定统计消息时，需要额外发送心跳
	if len(items) > 0 {
		// 发送monitor心跳，确保设备在cardagg中保持在线状态
...
func (c *MQTTConsumer) handleEventMessage(uid string, message map[string]interface{}) error {
	...
	for _, m := range items {
		...
		// 根据canMonitor标志决定如何处理事件数据
		if canMonitor {
			// CanIot=T & CanMonitor=T：正常发送事件数据，事件数据同时也作为心跳
			switch eventType {
			case 1:
			...
			// CanIot=T & CanMonitor=F：不发送事件数据，仅发送心跳信号
			// 直接跳过事件数据的处理
		}
	}

	// 无论canMonitor是true还是false，都需要确保发送心跳以维持设备在线状态
	// 当有人活动时，业务数据本身就作为心跳；当无人活动只有固定事件消息时，需要额外发送心跳
	if len(items) > 0 {
		// 发送monitor心跳，确保设备在cardagg中保持在线状态
		if err := c.publishRadarMonitorHeartbeat(ctx, tid, cid, uid, did, ts); err != nil {
			lastErr = err
