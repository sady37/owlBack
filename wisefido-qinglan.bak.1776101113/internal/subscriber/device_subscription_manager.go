...
type DeviceSubscription struct {
	...
	TenantID        string
	DeviceType      string
	// 添加sync.Once用于确保每个设备只执行一次首次消息处理
...
type DeviceSubscriptionManager struct {
	...
	subscriptionsByUID map[string]*DeviceSubscription // 通过UID查找
	subscriptionsByID  map[string]*DeviceSubscription // 通过ID查找
	mu                 sync.RWMutex
	logger             *zap.Logger
	defaultContent     int
	defaultDuration    int
	// 添加一个map来跟踪已经处理过首次消息的设备
	processedFirstMessage sync.Map
	// goroutine control
	wg       sync.WaitGroup
	...
	// stream publisher for publishing device status/alarm
	streamPublisher *consumer.StreamPublisher
	radarService    *service.RadarService
	// track forced-unsubscribed due to timeout
	unsubscribedDueToTimeout map[string]struct{}
}
...
func NewDeviceSubscriptionManager(
	...
	db *sql.DB,
	logger *zap.Logger,
	_messageHandler func(topic string, payload []byte) error,
) *DeviceSubscriptionManager {
	// defaultContent: 0 means subscribe all content
	content := 0

	// defaultDuration from subscription config (int)
	duration := cfg.Subscription.DefaultDuration
	if duration == 0 {
		duration = 3600 // 1 hour
	}

	monitorAge := time.Duration(duration) * time.Second
	if monitorAge == 0 {
		monitorAge = time.Hour
	}

	manager := &DeviceSubscriptionManager{
		config:                   cfg,
		db:                       db,
		...
		monitorMaxAge:            monitorAge,
		checkInterval:            90 * time.Second,
		unsubscribedDueToTimeout: make(map[string]struct{}),
	}

	// create deviceRepo locally from db to avoid needing it from caller
	manager.deviceRepo = repository.NewPostgresDeviceRepository(db)
...
// SetStreamPublisher sets StreamPublisher for publishing device status/alarm
func (m *DeviceSubscriptionManager) SetStreamPublisher(p *consumer.StreamPublisher) {
	m.streamPublisher = p
}

// SetMQTTConsumer sets the mqtt consumer (used to subscribe/unsubscribe topics)
func (m *DeviceSubscriptionManager) SetMQTTConsumer(c *consumer.MQTTConsumer) {
	m.mqttConsumer = c
}

// SetRadarService sets RadarService used by health checks
func (m *DeviceSubscriptionManager) SetRadarService(s *service.RadarService) {
	m.radarService = s
}
...
func (m *DeviceSubscriptionManager) SubscribeDevice(ctx context.Context, deviceUID, deviceID string) error {
	...
	if sub, exists := m.subscriptionsByUID[deviceUID]; exists {
		...
		// read monitorSubTime under lock
...
func (m *DeviceSubscriptionManager) autoSubscribeOnFirstMessage(ctx context.Context, deviceUID string) {
	// 使用sync.Map检查是否已经处理过此设备的首次消息
	if _, loaded := m.processedFirstMessage.LoadOrStore(deviceUID, struct{}{}); loaded {
		// 如果已经处理过，直接返回，避免重复处理
		return
	}
	...
	if err != nil {
		...
		// 删除标记，因为处理失败
		m.processedFirstMessage.Delete(deviceUID)
		return
	}
...
func (m *DeviceSubscriptionManager) unsubscribeDevice(deviceUID string) {
	...
	if m.mqttConsumer != nil {
		go m.mqttConsumer.UnsubscribeDeviceTopics(deviceUID)
	}
}
