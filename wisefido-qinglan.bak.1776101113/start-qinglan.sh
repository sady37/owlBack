...
# 等待之前的MQTT会话过期
echo "Waiting 3s for previous MQTT session to expire..."
sleep 3
...
echo ""
if go build -o "$BIN_DIR/wisefido-qinglan" ./cmd/wisefido-qinglan >/dev/null 2>&1; then
    if [ -t 1 ]; then
        "$BIN_DIR/wisefido-qinglan" 2>&1 | tee -a "$LOG_FILE"
    else
        "$BIN_DIR/wisefido-qinglan" 2>&1 | tee -a "$LOG_FILE"
    fi
else
