...
#
# 端点（前缀 /roomengine/* 避开 owlBack 的 /api/* auth middleware）：
#   GET /roomengine/health
#   GET /roomengine/playback?uid=&start=&end=&snap_min=&format=html|json[&layout=path]
#   GET /roomengine/playback/files?uid=
#   GET /roomengine/files/<filename>
...
# 等服务起来（最多 30s）
for i in {1..30}; do
	if curl -fs "http://127.0.0.1:$PORT/roomengine/health" >/dev/null 2>&1; then
		echo "✅ ready: http://127.0.0.1:$PORT/roomengine/health"
		echo ""
		echo "示例（在 alitest 上 curl 测试）:"
		echo "  curl 'http://127.0.0.1:$PORT/roomengine/playback?uid=9D8A326309E7&start=2026-04-25T13:00:00-07:00&end=2026-04-25T17:00:00-07:00&snap_min=10&layout=../doc/layout-09E7-room101.json' -o /tmp/x.html"
		echo ""
		echo "本地浏览器访问（先 ssh -L 7788:127.0.0.1:$PORT alitest）:"
		echo "  http://localhost:7788/roomengine/playback?uid=...&start=...&end=...&layout=../doc/layout-09E7-room101.json"
		exit 0
	fi
	sleep 1
done
