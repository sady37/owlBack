...
// publishInitialResetState 在 sensor 启动后主动 publish 所有 room/bed 的 vacant 初始态，
// 让 OOR/OOB anchor 从 now 起算，解决「sensor 重启后 cardagg card:state hash 保留 stale
// LastExitTime / StartTime（如 29h ago）」残留问题。
//
// 与 OnZoneEvent 路径互补——OnZoneEvent 只在 zone transition 时发，启动时无 transition；
// 本函数主动一次性发，确保重启即「刚开始观察」语义。
//
...
func publishInitialResetState(ctx context.Context, db *sql.DB, p *zoneengine.StreamPublisher, logger *zap.Logger) {
	...
	rRows, err := db.QueryContext(ctx, `SELECT room_id::text, COALESCE(room_type, 0) FROM rooms`)
	if err != nil {
	...
	for rRows.Next() {
		var cardID string
		var roomType int
		if err := rRows.Scan(&cardID, &roomType); err != nil {
			continue
		}
		rs := &card.RoomState{
			RoomType:     roomType,
	...
	for _, cardID := range bedCardIDs {
		bs := &card.BedState{
			UpdatedAt: nowMs,
			BedStatus: 1, // NotInBed — sensor 还没观察到任何 in-bed 信号
			StartTime: nowMs,
		}
