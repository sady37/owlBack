...
func (c *MonitorConsumer) Start(ctx context.Context) {
	if err := rediscommon.CreateConsumerGroup(ctx, c.client, rediscommon.StreamMonitor.Name, sensorMonitorGroup); err != nil {
		c.logger.Warn("sensor monitor: create consumer group", zap.Error(err))
	}
	go c.runLoop(ctx)
	c.logger.Info("sensor monitor consumer started",
		zap.String("stream", rediscommon.StreamMonitor.Name),
...
func (c *MonitorConsumer) runLoop(ctx context.Context) {
	for {
		...
		msgs, err := rediscommon.ReadFromStreamWithBlock(ctx, c.client,
			rediscommon.StreamMonitor.Name, sensorMonitorGroup, sensorMonitorConsumer,
		...
		for _, m := range msgs {
			...
			c.client.XAck(ctx, rediscommon.StreamMonitor.Name, sensorMonitorGroup, m.ID)
		}
	}
}
