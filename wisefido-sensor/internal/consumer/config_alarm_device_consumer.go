...
func (c *AlarmDeviceConfigConsumer) Start(ctx context.Context) {
	if err := rediscommon.CreateConsumerGroup(ctx, c.client,
		rediscommon.StreamConfigAlarmDevice.Name, sensorAlarmDeviceGroup); err != nil {
...
func (c *AlarmDeviceConfigConsumer) runLoop(ctx context.Context) {
	for {
		...
		msgs, err := rediscommon.ReadFromStreamWithBlock(ctx, c.client,
			rediscommon.StreamConfigAlarmDevice.Name, sensorAlarmDeviceGroup, sensorAlarmDeviceConsumer,
		...
		for _, m := range msgs {
			...
			c.client.XAck(ctx, rediscommon.StreamConfigAlarmDevice.Name, sensorAlarmDeviceGroup, m.ID)
		}
	}
}
