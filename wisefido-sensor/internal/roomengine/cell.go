...
type Cell struct {
	...
	// Chair 区久坐学习态不再挂 cell（cell index 随 layout 编辑漂）——迁到 per-object ChairDwell（chair_dwell.go，
	// keyed by canvas object_id，落 sibling 表 chair_dwell_state 跨编辑存活）。
...
func DefaultDecayParams() DecayParams {
	return DecayParams{
		...
		//   AreaActive (走道/淋浴)：           14 天衰到 10 HL=4.2d  — 路径稳定但易被新家具污染
		//   AreaInterfer (帘/扇/植物)：        14 天衰到 10 HL=4.2d  — 可移动
		//   AreaEnter (门洞)：                 30 天衰到 10 HL=9d    — 几何固定但 layout 可能改
		//   AreaDeny (家具)/AreaReflector (镜/金属)：30 天衰到 10 HL=9d — 固定
...
const (
	stillAreaDenySec       = 5 * 60 // AreaDeny/AreaInterfer
...
// 床/沙发：不限（休息合理）；Deny/Interfer: 5min；其它：8min。
