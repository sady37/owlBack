...
// BlindRest **无 Left 逃生阀**（rest 区非门，人不会从床里凭空离场，
// 离场靠 ObsEnterExit 从似然层拉 Left）；BlindOpen 有 Left 逃生阀（reachable-exit 放大）。
//
...
var transitionPropensity = Matrix{
	...
	SBlindRest: {0, 8, 0, 1, 6, 0.5, 84, 0, 0},        // 重现→Bed/Bath；★→Fallen 0.5 种子(靠 ObsNoDetect 放大非 A 漂)；**无 Left 逃生阀**
	SBlindOpen: {0, 0, 0, 8, 0, 0.5, 0, 71, 12},       // 重现→Open；→Left 12 逃生阀（reachable-exit 放大）；★→Fallen 0.5 种子
