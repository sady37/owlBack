...
type Observation struct {
	...
	// Chair 区 dwell 分布（box 起点锁定）：floor 只对 chair 区(InChair)按本格学到的久坐时长分布算连续阈
	//   tFloor = clamp(ChairMu + k·ChairSigma, [12min, 90min])；冷启(ChairMu≤0)回退 90min。
	...
	ChairMu     float64 // 本格 dwell EMA 均值（秒）；0=冷启(样本不足)
	ChairSigma  float64 // 本格 dwell EMA σ（秒）
