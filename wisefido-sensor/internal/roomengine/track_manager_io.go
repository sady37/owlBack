...
func (tm *TrackManager) PublishDBNFall(lid, band string, nowMs int64) {
	...
	if ts != nil {
		...
		fireEv := map[string]interface{}{
			...
			"chair_cap":         ts.StillBoxChairCap,
			"bath_mu":           bathMu,
			"bath_sigma":        bathSig,
			"bath_maxsit":       bathMaxSit,
			"tfloor_sec":        belief.TFloorFor(ts.LastCellArea, tm.roomType, IsNightTime(nowMs, tm.timezone), ts.StillBoxInChair, ts.StillBoxChairMu, ts.StillBoxChairSigma, ts.StillBoxChairMaxSit, ts.StillBoxChairCap, bathMu, bathSig, bathMaxSit),
