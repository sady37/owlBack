...
func (tm *TrackManager) PublishDBNFall(lid, band string, nowMs int64) {
	...
	if ts != nil {
		...
		fireEv := map[string]interface{}{
			...
			"tfloor_sec":        belief.TFloorFor(int(ts.LastCellArea), tm.roomType, IsNightTime(nowMs, tm.timezone), ts.StillBoxInChair, ts.StillBoxChairMu, ts.StillBoxChairMaxSit),
