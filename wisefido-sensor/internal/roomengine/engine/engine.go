...
func (r *Room) Tick(fi adapter.FrameInput, handoffL float64) Frame {
	...
	for _, ts := range tracks {
		...
		if ts.Present {
			...
			// Online=false 雷达 pose/z/area redirect 中性，但带续算 StillSec+AreaAt(① 冻结坐标)→
			...
			obs = adapter.BuildObservation(adapter.RadarTrack{
				...
				InChair: ts.Obs.RadarTrack.InChair, ChairMu: ts.Obs.RadarTrack.ChairMu, ChairSigma: ts.Obs.RadarTrack.ChairSigma, ChairMaxSit: ts.Obs.RadarTrack.ChairMaxSit, ChairCap: ts.Obs.RadarTrack.ChairCap,
