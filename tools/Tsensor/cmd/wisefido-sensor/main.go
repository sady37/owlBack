...
func main() {
	...
	// 2. 初始化日志 —— Tsensor:输出到 owl/log/Tsensor.log(不走 stdout,不改共享 logpkg)。
	logCfg := zap.NewProductionConfig()
	logCfg.OutputPaths = []string{"/home/wisefido/owl/log/Tsensor.log"}
	logCfg.ErrorOutputPaths = []string{"/home/wisefido/owl/log/Tsensor.log"}
	logger, err := logCfg.Build()
	if err != nil {
		panic(fmt.Sprintf("Failed to init logger: %v", err))
	}
	zap.ReplaceGlobals(logger) // Tsensor:让 zap.L()(如 alarm_back_channel)也写 Tsensor.log
	defer logger.Sync()
