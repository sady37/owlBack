...
func (c *AlarmConsumer) Start(ctx context.Context) {
	if err := rediscommon.CreateConsumerGroup(ctx, c.client, ("test:" + rediscommon.StreamAlarm.Name), sensorAlarmGroup); err != nil {
		c.logger.Warn("sensor alarm: create consumer group", zap.Error(err))
	}
	go c.runLoop(ctx)
	c.logger.Info("sensor alarm consumer started",
		zap.String("stream", ("test:" + rediscommon.StreamAlarm.Name)),
...
func (c *AlarmConsumer) runLoop(ctx context.Context) {
	for {
		...
		msgs, err := rediscommon.ReadFromStreamWithBlock(ctx, c.client,
			("test:" + rediscommon.StreamAlarm.Name), sensorAlarmGroup, sensorAlarmConsumer,
		...
		for _, m := range msgs {
			...
			c.client.XAck(ctx, ("test:" + rediscommon.StreamAlarm.Name), sensorAlarmGroup, m.ID)
		}
	}
}
