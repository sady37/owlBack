...
func (c *SleepStageConsumer) Start(ctx context.Context) {
	if err := rediscommon.CreateConsumerGroup(ctx, c.client, ("test:" + rediscommon.StreamEvent.Name), sensorSleepStageGroup); err != nil {
...
func (c *SleepStageConsumer) runLoop(ctx context.Context) {
	for {
		...
		msgs, err := rediscommon.ReadFromStreamWithBlock(ctx, c.client,
			("test:" + rediscommon.StreamEvent.Name), sensorSleepStageGroup, sensorSleepStageConsumer,
		...
		for _, m := range msgs {
			...
			c.client.XAck(ctx, ("test:" + rediscommon.StreamEvent.Name), sensorSleepStageGroup, m.ID)
		}
	}
}
