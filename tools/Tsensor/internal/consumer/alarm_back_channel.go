...
func (a *AlarmBackChannel) publishAlarm(
	...
	// Tsensor:发送改为输出到 Tsensor.log(不推 redis)。logged = 此 firmware alarm 被 forward
	// (即**未**被 dbn_mode 否决);否决的那条不会到这,据此核对 dbn_mode veto 行为。
	_ = ctx
	_ = values
	zap.L().Info("tsensor_output",
		zap.String("kind", "alarm_back_channel"),
		zap.String("category", eventName),
		zap.String("device_addr", deviceAddr.String()),
		zap.String("subject_entity", subjectEntity),
		zap.Int64("ts_ms", tsMs),
		zap.String("payload", string(dataValueJSON)),
	)
	return "", nil
}
