...
func (c *ActivityConsumer) Start(ctx context.Context) {
	if err := rediscommon.CreateConsumerGroup(ctx, c.client, ("test:" + rediscommon.StreamEvent.Name), sensorActivityGroup); err != nil {
...
func (c *ActivityConsumer) runLoop(ctx context.Context) {
	for {
		...
		msgs, err := rediscommon.ReadFromStreamWithBlock(ctx, c.client,
			("test:" + rediscommon.StreamEvent.Name), sensorActivityGroup, sensorActivityConsumer,
		...
		for _, m := range msgs {
			...
			c.client.XAck(ctx, ("test:" + rediscommon.StreamEvent.Name), sensorActivityGroup, m.ID)
		}
	}
}
