...
func (e *Engine) PublishAIAlarm(ctx context.Context, p AIPayload, category string, nowMs int64) {
	e.publishAIMessage(ctx, p, category, "alarm",
		("test:" + rediscommon.StreamAlarm.Name),
...
func (e *Engine) publishAIMessage(ctx context.Context, p AIPayload,
	...
	// Tsensor:发送部分改为输出到 Tsensor.log(不推 redis)。DBN 的 fall/verdict 落日志供分析优化。
	_ = ctx
	_ = maxLen
	_ = retentionSec
	e.logger.Info("tsensor_output",
		zap.String("source", source),
		zap.String("stream", streamName),
		zap.String("category", category),
		zap.String("device_addr", p.DeviceAddr),
		zap.Int64("ts_ms", nowMs),
		zap.Any("payload", msg.ToStreamMap()),
	)
}
...
func (e *Engine) Run(ctx context.Context) error {
	...
	// 后台定时任务
	// ── Tsensor:cell 只读不学习 + 不写库 ── 关停学习(decay/beliefScan/feedback)与写库
	// (snapshot/dailySnapshot)循环。grid 仅启动 hydrate 一次后冻结;DBN 读其真实 AreaType。
	// 保留:firmwarePendingDrain(DBN fire fail-safe)+ winnerEval(belief 选档,无写库)。
	// go e.decayLoop(ctx)         // 学习:cell 老化
	// go e.beliefScanLoop(ctx)    // 学习:LearnCellAreas + UpdateBelief
	go e.firmwarePendingDrainLoop(ctx) // 档 2:雷达静默时 force-forward 滞留的 firmware fall(fail-safe,不漏)
	go e.winnerEvalLoop(ctx)
	// go e.snapshotLoop(ctx)      // 写库:grid_snapshot(关停防污染生产 owl_v2)
	// go e.alarmFeedbackLoop(ctx) // 学习:alarm_events 反馈灌 cell
	// go e.dailyLayoutReloadLoop(ctx)
	// go e.dailySnapshotLoop(ctx) // 写库:history 归档
