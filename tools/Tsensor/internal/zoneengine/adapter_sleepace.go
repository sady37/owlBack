...
func (a *SleepaceAdapter) Start(ctx context.Context) {
	if err := rediscommon.CreateConsumerGroup(ctx, a.client, ("test:" + rediscommon.StreamEvent.Name), sleepaceConsumerGroup); err != nil {
...
func (a *SleepaceAdapter) runLoop(ctx context.Context) {
	for {
		...
		msgs, err := rediscommon.ReadFromStreamWithBlock(ctx, a.client,
			("test:" + rediscommon.StreamEvent.Name), sleepaceConsumerGroup, sleepaceConsumerName,
		...
		for _, m := range msgs {
			...
			a.client.XAck(ctx, ("test:" + rediscommon.StreamEvent.Name), sleepaceConsumerGroup, m.ID)
		}
	}
}
