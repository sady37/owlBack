...
// ForceReal 有出生证 → real：判决 latch 置真 + 显示满格。
func (r *RealnessTrack) ForceReal() { r.proven, r.pReal = true, 1 }

// ForceGhost 反证法：无出生证 → ghost 判决（proven=false）。显示 pReal 由 census 按 R 分档另设（SetDisplay），不在此动。
func (r *RealnessTrack) ForceGhost() { r.proven = false }

// SetDisplay 设无证轨显示 confidence（census 按离 born 最远净距 R 分档；纯 FE，gate 任何决策皆无）。
