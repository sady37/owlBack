...
type NeighborParams struct {
	HandoffWindowMs int64   // W：hand-off 检测窗上界（规则④随公共度收小）
	JitterMs        int64   // J：反向余量（时钟抖动容差）
	TauPeakMs       float64 // band-pass 峰时（~1-5s，老人跨房步速）
...
// wDir §A.1 有向 band-pass 核（规则③：同 tick 压低非零、峰 1-5s、后衰减；对 sign(Δ) 不对称）。
func wDir(p NeighborParams, deltaMs int64) float64 {
	if deltaMs < -p.JitterMs || deltaMs > p.HandoffWindowMs {
		return 0 // 窗外 / 真反向 → 非 hand-off（stale 证不了此刻在哪）
	}
	if deltaMs >= 0 {
		// 先升后降：x=(Δ+offset)/τp，Δ=0→x=offset/τp（小，压低非零），峰在 Δ=τp−offset（~1-5s）。
		return gammaPeak1((float64(deltaMs) + p.OffsetMs) / p.TauPeakMs)
	}
	// −J ≤ Δ < 0：从 Δ=0 的压低值按 τj 再衰减（抖动反向余量）。
	return gammaPeak1(p.OffsetMs/p.TauPeakMs) * math.Exp(float64(deltaMs)/p.TauJMs)
}
...
func RhoXroom(p NeighborParams, residentCount int, sibs []SiblingHandoff) float64 {
	...
	best := 0.0
	for _, s := range sibs {
		if v := wDir(p, s.ArrivalDeltaMs) * s.CAttr * s.GainedReal; v > best {
			best = v
		}
	}
	rho := eta * best
