...
func ParseLayoutConfig(roomID string, layoutJSON []byte) (RoomConfig, error) {
	...
	for _, objRaw := range layout.Objects {
		var hdr struct {
			...
			EnterTarget string          `json:"enter_target,omitempty"`
		}
		...
		// ObjectType 大小写不敏感。rules.md #S2：ObjectType 只做物理归类，不定空间语义。
		...
		// 空间轴单源：每个区域对象一条 AreaZone（AreaType 跟对象走）。Chair 置信 80，其余 99。
