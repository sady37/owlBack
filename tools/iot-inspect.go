...
func main() {
	var (
		...
		stream    = flag.String("stream", "iot:monitor:stream", "Redis Stream 名称 (仅当 source=redis 时使用，支持 iot:*:stream)")
	...
	case "db":
		...
		if len(dbRecords) == 0 {
			...
			if *topicType != "" {
				...
				fmt.Fprintf(os.Stderr, "  4. Table structure may use different field names (raw_original vs data_values)\n")
			}
			// 不退出，只是提示
...
func ReadFromDatabase(ctx context.Context, dbConfig *commonconfig.DatabaseConfig, topicType, deviceID string, limit int) ([]DatabaseRecord, error) {
	...
	// 查询字段（兼容实际表结构：使用 raw_original 和 metadata）
	baseQuery := `
		SELECT 
			id,
			device_id,
			...
			CASE 
				...
				THEN convert_from(raw_original, 'UTF8')::jsonb
				ELSE '{}'::jsonb
			END as data_values,
			CASE 
				WHEN raw_original IS NOT NULL 
	...
	if topicType != "" {
		query += fmt.Sprintf(" AND raw_original IS NOT NULL AND convert_from(raw_original, 'UTF8')::jsonb->>'topic_type' = $%d", argIndex)
	...
	for rows.Next() {
		...
		// 从 data_values 中提取字段
		if deviceUID, ok := record.DataValues["device_uid"].(string); ok && deviceUID != "" {
			record.DeviceUID = &deviceUID
		}
		if topicTypeVal, ok := record.DataValues["topic_type"].(string); ok && topicTypeVal != "" {
			record.TopicType = &topicTypeVal
		}
		if category, ok := record.DataValues["category"].(string); ok && category != "" {
			record.Category = &category
		}
		if branchID, ok := record.DataValues["branch_id"].(string); ok && branchID != "" {
			record.BranchID = &branchID
		}
		if buildingID, ok := record.DataValues["building_id"].(string); ok && buildingID != "" {
			record.BuildingID = &buildingID
		}
		if unitID, ok := record.DataValues["unit_id"].(string); ok && unitID != "" {
			record.UnitID = &unitID
		}
		if roomID, ok := record.DataValues["room_id"].(string); ok && roomID != "" {
			record.RoomID = &roomID
		}
		if bedID, ok := record.DataValues["bed_id"].(string); ok && bedID != "" {
			record.BedID = &bedID
		}
