...
// 默认渲染到 t.TempDir()，仅在 `go test -update-roomsvg` 时覆盖 doc/ 下的快照。
// Why: 之前每次跑 ./... 都会改写 doc/room-*.svg，污染 git working tree。
var updateRoomSVG = flag.Bool("update-roomsvg", false, "overwrite doc/room-*.svg snapshots")

func renderRoomLayoutSVG(t *testing.T, roomID, layoutPath, docPath string) {
	t.Helper()
	outPath := docPath
	if !*updateRoomSVG {
		outPath = filepath.Join(t.TempDir(), filepath.Base(docPath))
	}
