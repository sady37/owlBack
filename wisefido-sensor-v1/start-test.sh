...
# wisefido-sensor 测试启动脚本
# 使用测试配置启动 AI 智能推理服务

set -e

echo "========================================="
echo "Starting wisefido-sensor (sensor agent layer 1)"
echo "========================================="
echo ""
...
# TENANT_ID 可选（不设也能起 wisefido-sensor；建业务租户后再填）
if [ -z "$TENANT_ID" ]; then
...
echo ""
echo -e "${BLUE}服务配置:${NC}"
echo "  服务名称: wisefido-sensor (sensor agent layer 1)"
echo "  功能范围:"
echo "    - 高级推理报警（事件1, 3, 4）"
echo "    - 访客识别和智能分析"
echo "    - 巡房优化和模式识别"
...
BIN_DIR="$PWD/.bin"
mkdir -p "$BIN_DIR"
if go build -o "$BIN_DIR/wisefido-sensor" ./cmd/wisefido-sensor >/dev/null 2>&1; then
    "$BIN_DIR/wisefido-sensor"
else
    echo -e "${YELLOW}Warning: go build failed, falling back to go run${NC}"
    go run cmd/wisefido-sensor/main.go
fi