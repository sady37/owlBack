-- 检查 demo 租户 admin 用户的 PIN hash
-- 使用方法: psql -U your_user -d your_database -f check_pin_hash.sql

-- 1. 查找 demo 租户的 tenant_id
SELECT 
    tenant_id::text as tenant_id,
    tenant_name
FROM tenants 
WHERE tenant_name = 'demo';

-- 2. 查找 demo 租户中 admin 用户的 PIN hash
SELECT 
...
-- 3. 计算 PIN 1212 和 1234 的 SHA256 hash (用于对比)
...
-- 4. 对比: 检查数据库中存储的 PIN hash 是否匹配 1212 或 1234
SELECT 
    u.user_id::text as user_id,
    u.user_account,
