...
func NewRadarService(cfg *config.Config, logger *zap.Logger) (*RadarService, error) {
	...
	// 创建配置消费者（订阅配置变更流，清除报警使能缓存）
	configConsumer := consumer.NewConfigConsumer(cfg, redisClient, deviceRepo, logger)
...
func (s *RadarService) Start(ctx context.Context) error {
	...
	// 启动配置消费者（后台运行，清除报警使能缓存）
	if s.configConsumer != nil {
		go func() {
			if err := s.configConsumer.Start(ctx); err != nil {
				s.logger.Error("Config consumer error", zap.Error(err))
				// 配置消费者失败不应影响主流程
			}
		}()
		s.logger.Info("Config consumer started")
	}
