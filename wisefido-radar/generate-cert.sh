...
# 服务器信息（可以根据实际部署环境修改）
COMMON_NAME="${CERT_CN:-10.0.0.30}"  # 服务器 IP 或域名
ORG_NAME="${CERT_ORG:-WiseFido}"
COUNTRY="${CERT_COUNTRY:-US}"

echo "🔐 Generating self-signed certificate for Radar HTTPS Auth Server"
echo "================================================================"
...
echo "   export RADAR_HTTPS_CERT_FILE=$CERT_DIR/$CERT_FILE"
echo "   export RADAR_HTTPS_KEY_FILE=$CERT_DIR/$KEY_FILE"
echo ""
echo "2. Or update start-radar.sh to use these certificate files"
echo ""
