...
echo "  - wisefido-card-manage: Full card creation on startup (in main.go)"
...
cleanup() {
    ...
    pkill -f "wisefido-card-manage" || true
...
# 启动 wisefido-card-manage 服务
# 功能：卡片管理服务（提供 HTTP API 用于创建/更新卡片）
...
CARD_MANAGE_PID=$!
echo "  PID: $CARD_MANAGE_PID"
echo "  Log: $CARD_MANAGE_LOG (also displayed in terminal)"
...
if [ "$CARD_MANAGE_FAILED" = true ]; then
    echo -e "${YELLOW}Warning: wisefido-card-manage failed, but continuing with other services${NC}"
    echo "  You can check the log: $CARD_MANAGE_LOG"
    echo ""
fi
