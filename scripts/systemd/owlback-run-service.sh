...
# wisefido-sensor DBN cutover 三档（可逆，运维侧 .env 翻 DBN_MODE；默认 1=保 firmware 地板）。
#   DBN_MODE=0 → 否决 firmware fire + DBN 不自发（DBN 只当 firmware ghost 过滤器）
#   DBN_MODE=1 → 不否决 firmware fire + DBN 自发（firmware 地板不可挡,union；默认）
#   DBN_MODE=2 → 否决 firmware fire + DBN 自发（全开）
#   DBN_VETO_RECOVERY=1 → 启 recovery-veto 抑制纯误火（漏报-safe by construction；默认 0）。
export DBN_MODE="${DBN_MODE:-1}"
