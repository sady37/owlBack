...
`room_name` 主要是在 `wisefido-ai` 中，用于判断 room 是不是 bathroom（卫生间）。
...
**实现**：
```go
// wisefido-ai 从 cards.devices JSONB 读取设备信息
...
### 2. `wisefido-ai` 的 `DeviceInfo` 包含 `RoomName`
...
在 `wisefido-ai` 的 Evaluator 层（事件3）中：
```go
...
✅ **当前实现是正确的**：
- `cards.devices` JSONB 中包含 `room_name`，方便 `wisefido-ai` 直接使用
- 避免 alarm 需要额外查询 `rooms` 表
- 提供预计算数据，提高性能

✅ **`room_name` 的主要用途**：
- 在 `wisefido-ai` 中判断房间是否是 bathroom（事件3）
- 也可以用于前端显示（如果需要）
