...
### 1. 在 wisefido-radar 中处理雷达事件

**位置**：`wisefido-radar/internal/consumer/mqtt_consumer.go`（使用 `owl-common/encode.RadarEncode`）
...
### 阶段1：在 wisefido-radar 中处理雷达事件（使用 owl-common/encode）
