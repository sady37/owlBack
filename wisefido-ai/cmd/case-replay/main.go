...
// 让 wisefido-ai 在线服务实时消费，用于端到端验证 wire schema / B1+B2 修复 /
// ghost 算法路径等。
//
// 与 cmd/roomengine-playback 区别：
//   - playback：直接构造 TrackManager + grid 跑学习/检测，**不发 redis**，
//     用于离线分析与生成 HTML 查看器。
//   - case-replay：写 redis stream，让正在跑的 wisefido-ai 服务接收，
...
// 验证流程：
//  1. 启动本工具回放
//  2. 期间 tail -f /home/wisefido/owl/log/wisefido-ai.log | grep ai_emit
...
func main() {
	var (
		...
		tenantID   = flag.String("tenant-id", "", "tenant_id（必填，wisefido-ai 路由查 tenant 用）")
	...
	log.Printf("next: tail -f /home/wisefido/owl/log/wisefido-ai.log | grep ai_emit")
