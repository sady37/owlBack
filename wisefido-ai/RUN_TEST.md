...
# 方式2：编译后运行
go build -o wisefido-ai cmd/wisefido-ai/main.go
./wisefido-ai
```
...
2. 检查缓存键格式是否正确
3. 确认是否运行了 `wisefido-card-aggregator`（包含传感器融合功能）
...
# 如果没有数据，运行 wisefido-card-aggregator（包含传感器融合功能）
cd /Users/sady3721/project/owlBack/wisefido-card-aggregator
go run cmd/wisefido-card-aggregator/main.go
```
...
```bash
# 查看错误日志
go run cmd/wisefido-ai/main.go 2>&1 | grep -i error

# 查看报警事件创建日志
go run cmd/wisefido-ai/main.go 2>&1 | grep "Alarm event created"
```
