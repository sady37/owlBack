...
**⚠️ 视觉降饱和实现踩坑（2026-05-06 实测）**：
- 姿态 SVG 图标 `fill="#515151"` **本身就是纯灰色** → `filter: saturate(N%)` 对纯灰色完全无效（饱和度 0 × 任何系数 = 还是 0）
- 用户验证 `saturate(30%)` 跟原图视觉一模一样，要求换方案
- 现已改为 `filter: opacity(0.3)` —— ghost 直接变淡半透明，对比明显
- RadarCanvas.vue:2046 当前实现：`if (isLowConf && !isFall) ctx.filter = 'opacity(0.3)'`
- **教训**：以后给单色 SVG 做"降饱和度"视觉，必须用 opacity / brightness / 染色，不要用 saturate
