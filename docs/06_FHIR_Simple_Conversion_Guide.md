...
- **`behavioral`**: 行为健康告警（离床、床上坐起、异常体动、上床等）。
     注意：某些事件（如上床），（滞留、24H无人）是否触发告警取决于住户的 `service_level`。当 `service_level` 要求关注此类行为时，应归类为 `behavioral` 并使用 Flag 资源；否则可作为 `activity` 使用 Observation 资源记录
