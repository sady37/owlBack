...
func (c *MQTTConsumer) handleRealtimeData(msg *models.ReceivedMessage, device *repository.Device) error {
	...
	// 1. 查询设备位置信息（branch_id, building_id, unit_id, room_id, bed_id）
	locationInfo, err := c.getDeviceLocationInfo(context.Background(), device)
	if err != nil {
	...
	dataValue["category"] = "realtime"
	
	// 添加实时数据字段
	dataValue["breath"] = realtimeData.Breath
	dataValue["heart"] = realtimeData.Heart
	dataValue["turnOver"] = realtimeData.TurnOver
	dataValue["bodyMove"] = realtimeData.BodyMove
	dataValue["sitUp"] = realtimeData.SitUp
	dataValue["initStatus"] = realtimeData.InitStatus
	dataValue["bedStatus"] = realtimeData.BedStatus
	dataValue["signalQuality"] = realtimeData.SignalQuality
	dataValue["leftRight"] = realtimeData.LeftRight

	// 3. 构建完整的输出对象（按标准格式）
	// 字段顺序：device_id → device_type → tenant_id → timestamp → topic_type → category → data_value → 位置信息
	encodedData := map[string]interface{}{
		...
		"topic_type":  "monitor",
		"category":    "realtime",  // 从 data_value 提取到顶层
