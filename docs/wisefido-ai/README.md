# wisefido-ai - AI智能推理服务

## 📋 简介

`wisefido-ai` 是AI智能推理层服务，负责**高级智能分析和推理**：
...
- **设备直接报警**（由 `wisefido-card-aggregator` 处理）：
...
- Redis
- 前置服务：
  - `wisefido-card-aggregator` - 创建卡片和生成实时数据
  - `wisefido-iot-timeseries` - 存储设备数据并发布到 iot:data:stream
