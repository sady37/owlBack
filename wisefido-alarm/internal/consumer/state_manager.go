...
type Event1State struct {
	...
	// 位置跟踪（用于检测移动）
	LastPosition *struct {
		X int `json:"x"` // radar_pos_x（厘米）
		Y int `json:"y"` // radar_pos_y（厘米）
	} `json:"last_position,omitempty"` // 最后记录的位置
	LastPositionTime *int64 `json:"last_position_time,omitempty"` // 最后位置的时间戳

	// T0+10秒：track消失检测
	TrackDisappearTime *int64 `json:"track_disappear_time,omitempty"` // track消失的时间（如果存在）
	NewTrackAppeared   bool   `json:"new_track_appeared"`             // 是否有新的可移动track出现

	// 报警状态
	AlarmTriggered bool `json:"alarm_triggered"` // 是否已触发报警（避免重复报警）
}
...
type Event3State struct {
	TrackID          string  `json:"track_id"`                    // 跟踪的 track_id
	EnterTime        *int64  `json:"enter_time,omitempty"`        // T0：进入 bathroom 的时间
	StandingTime     *int64  `json:"standing_time,omitempty"`     // 开始站立的时间
	LastPosition     *struct {
		X float64 `json:"x"`
		Y float64 `json:"y"`
	} `json:"last_position,omitempty"` // 最后位置
	LastPositionTime *int64  `json:"last_position_time,omitempty"` // 最后位置的时间
	PositionChange   float64 `json:"position_change"`             // 位置变化（cm）
	StandingDuration int64   `json:"standing_duration"`            // 站姿持续时间（秒）
	StillDuration    int64   `json:"still_duration"`               // 位置不动持续时间（秒）
	AlarmTriggered   bool    `json:"alarm_triggered"`              // 是否已触发报警（避免重复报警）
}
